...
internal sealed class PrivacyWindow : Window, IDisposable
{
    ...
    private void DrawContactTexts(PrivateContact contact, Vector2 pos, float width)
    {
        ...
        var locationColor = isOffline ? Alpha(UiColors.TextDim, 0.32f) : UiColors.TextDim;
        drawList.PushClipRect(locationPos - new Vector2(0f, 1f * scale), new Vector2(locationPos.X + safeWidth, locationPos.Y + 17f * scale), true);
        DrawTextWithShadow(drawList, locationPos, Color(locationColor), FormatContactLocation(contact), 14.2f * scale);
        drawList.PopClipRect();
    ...
    private void DrawContactNameLine(PrivateContact contact, Vector2 pos, float width)
    {
        ...
        if (hasSymbol)
        {
            var symbolColor = isOffline ? Alpha(ResolveSymbolColor(contact), offlineTextAlpha) : ResolveSymbolColor(contact);
            DrawTextWithShadow(drawList, nameStart, Color(symbolColor), symbolText, nameFontSize);
            nameStart.X += symbolWidth;
        }

        drawList.PushClipRect(nameStart - new Vector2(0f, 2f * scale), new Vector2(nameStart.X + visibleNameWidth, nameStart.Y + 23f * scale), true);
        var displayNameColor = contact.HasSoftWarning ? UiColors.Busy : UiColors.Text;
        if (isOffline)
            displayNameColor = Alpha(displayNameColor, offlineTextAlpha);
        DrawTextWithShadow(drawList, nameStart, Color(displayNameColor), displayName, nameFontSize);
        drawList.PopClipRect();
        ...
        {
            var tagPos = new Vector2(cursorX, pos.Y + 2f * scale);
            var tagColor = isOffline ? Alpha(ResolveTagColor(contact), offlineTextAlpha) : ResolveTagColor(contact);
            drawList.PushClipRect(tagPos - new Vector2(0f, 1f * scale), new Vector2(tagPos.X + maxTagWidth, tagPos.Y + 19f * scale), true);
            DrawTextWithShadow(drawList, tagPos, Color(tagColor), tagText, tagFontSize);
            drawList.PopClipRect();
            cursorX += maxTagWidth + gap;
        ...
        if (hasGroup && groupInfo.group != null)
        {
            ...
            if (isOffline)
                groupColor = Alpha(groupColor, offlineTextAlpha);
        ...
        if (contact.HasSoftWarning)
        {
            using (ImRaii.PushFont(UiBuilder.IconFont))
            {
                ...
                var warningColor = isOffline ? Alpha(UiColors.Busy, offlineTextAlpha) : UiColors.Busy;
                drawList.AddText(UiBuilder.IconFont, 8.8f * scale, new Vector2(cursorX, pos.Y + 5f * scale), Color(warningColor), warnIcon);
            }
            cursorX += 13f * scale;
        }
        ...
        using (ImRaii.PushFont(UiBuilder.IconFont))
        {
            ...
            if (isOffline)
                color = Alpha(color, offlineTextAlpha);
        ...
        using (ImRaii.PushFont(UiBuilder.IconFont))
        {
            ...
            if (isOffline)
                pinColor = Alpha(pinColor, offlineTextAlpha);
    ...
    private void DrawTopSummaryPanel()
    {
        ...
        // Reserve real layout space for the top bar before the main content is drawn.
        // The visible bar itself is drawn at the end of Draw() in a child overlay layer,
        // so the main content/background cannot draw over it.
        ImGui.SetCursorScreenPos(cursor + new Vector2(0f, height + 6f * scale));
    }
    ...
    private void RedrawTopSummaryPanelOverlay()
    {
        ...
        var borderThickness = MathF.Max(1f, 1.15f * scale);
        ...
        var borderColor = Color(Alpha(config.AccentColor, CompactSidebarRailBorderAlpha + 0.18f));
        drawList.AddRect(pos, pos + size, borderColor, rounding, ImDrawFlags.RoundCornersAll, borderThickness);
        ...
        var mouse = ImGui.GetMousePos();
        var hoveredCollapse = mouse.X >= collapsePos.X && mouse.X <= collapsePos.X + closeSize.X && mouse.Y >= collapsePos.Y && mouse.Y <= collapsePos.Y + closeSize.Y;
        ...
        var hoveredClose = mouse.X >= closePos.X && mouse.X <= closePos.X + closeSize.X && mouse.Y >= closePos.Y && mouse.Y <= closePos.Y + closeSize.Y;
        ...
        HandleTopSummaryPanelButtons(collapsePos, closePos, closeSize);
    }

    private void HandleTopSummaryPanelButtons(Vector2 collapsePos, Vector2 closePos, Vector2 buttonSize)
    {
        var mouse = ImGui.GetMousePos();
        var overCollapse = mouse.X >= collapsePos.X && mouse.X <= collapsePos.X + buttonSize.X &&
                           mouse.Y >= collapsePos.Y && mouse.Y <= collapsePos.Y + buttonSize.Y;
        var overClose = mouse.X >= closePos.X && mouse.X <= closePos.X + buttonSize.X &&
                        mouse.Y >= closePos.Y && mouse.Y <= closePos.Y + buttonSize.Y;

        if (overCollapse && ImGui.IsMouseClicked(ImGuiMouseButton.Left))
        {
            config.WindowCollapsed = !config.WindowCollapsed;
            config.Save();
        }

        if (overClose && ImGui.IsMouseClicked(ImGuiMouseButton.Left))
            IsOpen = false;
    }
